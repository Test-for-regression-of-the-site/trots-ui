...
class DetailsRenderer {
    ...
    switch (details.type) {
      ...
      case "bytes":
        return this._renderBytes(
          /** @type {NumericUnitDetailsJSON} */ (details)
        );
      case "ms":
  ...
  _renderTable(details) {
    ...
    for (const heading of details.headings) {
      ...
      this._dom.createChildOf(theadTrElem, "th", classes).appendChild(
        this.render({
          type: "text",
          value: heading.text || ""
        })
    ...
    for (const row of details.items) {
      ...
      for (const heading of details.headings) {
        ...
        const value =
          /** @type {number|string|DetailsJSON|undefined} */ (row[key]);
        ...
        if (value.type) {
          ...
          this._dom
            ...
            .appendChild(this.render(valueAsDetails));
          continue;
        }
  ...
  _renderOpportunityTable(details) {
    ...
    for (const row of details.items) {
      ...
      for (const heading of details.headings) {
        const key =
          /** @type {keyof LH.Audit.Details.OpportunityItem} */ (heading.key);
        const value = row[key];
